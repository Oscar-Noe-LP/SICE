...
class CarreraController extends Controller
{
        ...
        $carreras = DB::table('carrera as c')
            ...
            ->map(function ($c) {
                return [
                    ...
                    'departamento'    => ['nombre' => $c->departamento_nombre],
                    'nivel'           => ['nombre_nivel' => $c->nombre_nivel]
                ];
            });
    ...
    public function store(Request $request)
    {
        ...
        return response()->json(['message' => 'Carrera creada', 'id' => $id], 201);
    }
    ...
    {
        DB::table('carrera')->where('id_carrera', $id)->delete();
        return response()->json(['message' => 'Carrera eliminada']);
    }

    // ─────────────────────────────────────────────
    // PANTALLA 2 (drill-down completo): grupos agrupados por semestre y clave_grupo
    // GET /api/carreras/{id}/grupos-agrupados
    //
    // Resultado por fila: semestre, clave_grupo, total_materias, inscritos, capacidad
    public function gruposAgrupados($id)
    {
        ...
        // Ejecutar el query agrupado tal como se especificó, compatible con MySQL < 8
        // (usa subconsulta correlacionada en lugar de SUM OVER PARTITION para mayor compatibilidad)
        $filas = DB::select("
            SELECT
                pm.semestre,
                g.clave_grupo,
                COUNT(g.id_grupo)                                          AS total_materias,
                (
                    SELECT COUNT(*)
                    FROM inscripcion i2
                    JOIN grupo g2 ON g2.id_grupo = i2.id_grupo
                    WHERE g2.clave_grupo = g.clave_grupo
                      AND i2.estatus = 'Activo'
                )                                                          AS inscritos,
                g.capacidad
            FROM plan_estudio pe
            JOIN plan_materia pm  ON pm.id_plan   = pe.id_plan
            JOIN materia m        ON m.id_materia = pm.id_materia
            JOIN grupo g          ON g.id_materia = m.id_materia
            WHERE pe.id_carrera = ?
              AND pe.estatus    = 1
              AND g.estatus     = 1
            GROUP BY pm.semestre, g.clave_grupo, g.capacidad
            ORDER BY pm.semestre, g.clave_grupo
        ", [$id]);

        // Agrupar las filas por semestre para el frontend
        $porSemestre = [];
        foreach ($filas as $fila) {
            $sem = (int) $fila->semestre;
            if (!isset($porSemestre[$sem])) {
                $porSemestre[$sem] = [
                    'semestre' => $sem,
                    'grupos'   => []
                ];
            }
            $porSemestre[$sem]['grupos'][] = [
                'clave_grupo'    => $fila->clave_grupo,
                'total_materias' => (int) $fila->total_materias,
                'inscritos'      => (int) $fila->inscritos,
                'capacidad'      => (int) $fila->capacidad,
                // porcentaje de ocupación listo para la barra de progreso
                'ocupacion'      => $fila->capacidad > 0
                    ? round(($fila->inscritos / $fila->capacidad) * 100)
                    : 0,
            ];
        }
        ...
        return response()->json([
            ...
            'semestres'    => array_values($porSemestre),  // array indexado para JS
        ]);
    }

    // ─────────────────────────────────────────────
    // PANTALLA 2 (original): Drill-down de grupos por carrera
    ...
    public function grupos($id)
    {
        ...
        if ($materiasConSemestre->isEmpty()) {
            return response()->json(['carrera' => $carrera->nombre, 'grupos' => []]);
        }
        ...
        $grupos = DB::table('grupo as g')
            ...
            ->select(
                ...
                'g.capacidad',
                DB::raw("(SELECT COUNT(*) FROM inscripcion AS i
                          WHERE i.id_grupo = g.id_grupo AND i.estatus = 'Activo') as inscritos")
            )
            ->orderBy('m.nombre')
            ->get()
        ...
        return response()->json(['carrera' => $carrera->nombre, 'grupos' => $grupos]);
    }
    ...
    public function gruposPorSemestre($id, $semestre)
    {
        ...
        $grupos = DB::table('grupo as g')
            ...
            ->select(
                ...
                DB::raw("ROUND((SELECT AVG(sub.cal_pond)
                    FROM (SELECT SUM(c.calificacion * ev.porcentaje / 100) as cal_pond
                          FROM inscripcion i
                          JOIN calificacion c  ON c.id_inscripcion = i.id_inscripcion
                          JOIN evaluacion ev   ON ev.id_evaluacion = c.id_evaluacion
                          WHERE i.id_grupo = g.id_grupo
                          GROUP BY i.id_inscripcion) sub), 1) as promedio")
            )
