          ...
          <tbody>
            ...
            >
              ...
              <td class="celda-acciones">
                <!-- Ver -->
                <button class="btn-icono ver" @click.stop="abrirModalVer(puesto)" title="Ver detalles">
                ...
                <!-- Editar -->
                <button class="btn-icono editar" @click.stop="abrirModalEditar(puesto)" title="Editar puesto">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
import { getPuestos, crearPuesto, actualizarPuesto, eliminarPuesto as eliminarPuestoAPI } from '@/api/recursosHumanos'
...
const guardarPuesto = async () => {
  ...
  try {
    const datos = { nombre_puesto: puesto.nombre_puesto, descripcion: puesto.descripcion }
    if (puesto.id_puesto) {
      const res = await actualizarPuesto(puesto.id_puesto, datos)
      if (res.success) { mostrarNotificacion('Puesto actualizado', 'exito'); cerrarModal(); cargarPuestos() }
    } else {
      const res = await crearPuesto(datos)
      if (res.success) { mostrarNotificacion('Puesto creado', 'exito'); cerrarModal(); cargarPuestos() }
    }
  } catch (error) {
    console.error(error)
...
const eliminarPuesto = async (id) => {
  ...
  try {
    ...
    if (res.success) { mostrarNotificacion('Puesto eliminado', 'exito'); cargarPuestos() }
    else if (res.error?.includes('empleado')) { mostrarNotificacion('No se puede eliminar: tiene empleados asignados', 'error') }
  } catch (error) {
    console.error(error)
...
.search-group { position: relative; flex: 0 0 300px; min-width: 200px; }
...
.puestos-table th { background: var(--fondo); padding: 8px 12px; text-align: left; font-weight: 600; font-size: 0.8rem; color: var(--texto); border-bottom: 1px solid var(--borde); font-family: 'Montserrat', sans-serif; white-space: nowrap; }
.puestos-table td { padding: 7px 12px; border-bottom: 1px solid var(--borde); color: var(--texto); font-size: 0.86rem; font-family: 'Montserrat', sans-serif; }
...
.celda-desc { color: var(--gris); font-size: 0.84rem; max-width: 400px; }
...
.modal-content { background: #FFFFFF; width: 460px; max-width: 92%; border-radius: 12px; box-shadow: 0 20px 50px rgba(0,0,0,0.18); overflow: hidden; border: 1px solid var(--borde); }
...
.modal-input { width: 100%; padding: 8px 12px; border: 1.5px solid var(--borde); border-radius: 7px; font-size: 0.9rem; background: #FFFFFF; color: var(--texto); font-family: 'Montserrat', sans-serif; outline: none; transition: border-color 0.2s; box-sizing: border-box; }
.modal-input:focus { border-color: #1B396A; }
.modal-textarea { width: 100%; padding: 8px 12px; border: 1.5px solid var(--borde); border-radius: 7px; font-size: 0.9rem; background: #FFFFFF; color: var(--texto); font-family: 'Montserrat', sans-serif; outline: none; transition: border-color 0.2s; box-sizing: border-box; resize: vertical; min-height: 80px; }
...
.detalle-fila { display: flex; justify-content: space-between; align-items: flex-start; padding: 8px 0; border-bottom: 1px solid var(--borde); font-size: 0.9rem; color: var(--texto); font-family: 'Montserrat', sans-serif; gap: 1rem; }
