            ...
            <div v-if="mostrarFiltros" class="filtros-panel" @click.stop>
              ...
              <div class="filtros-panel-body">
                <div class="campo-filtro">
                  <label>Puesto</label>
                  <select v-model="filtroPuesto" class="filtro-select" @change="currentPage = 1">
          ...
          <tbody>
            ...
            >
              ...
              <td class="celda-acciones">
                <!-- Ver -->
                <button class="btn-icono ver" @click.stop="abrirModalVer(empleado)" title="Ver detalles">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const filtrosActivos = computed(() =>
  !!filtroPuesto.value || !!filtroDepartamento.value || !!filtroEstatus.value
)
const contadorFiltros = computed(() =>
  [filtroPuesto.value, filtroDepartamento.value, filtroEstatus.value].filter(Boolean).length
)
...
.btn-filtros { display: flex; align-items: center; gap: 7px; background: #FFFFFF; color: #1A1A1A; border: 1px solid #E5E7EB; padding: 8px 14px; border-radius: 8px; font-weight: 600; cursor: pointer; font-size: 0.88rem; font-family: 'Montserrat', sans-serif; transition: background 0.15s, border-color 0.15s; white-space: nowrap; position: relative; }
...
.filtros-panel { position: absolute; top: calc(100% + 8px); left: 0; width: 280px; background: #FFFFFF; border: 1px solid #E5E7EB; border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.12); z-index: 500; overflow: hidden; }
...
.filter-select { padding: 7px 10px; border: 1px solid var(--borde); border-radius: 7px; font-size: 0.86rem; flex: 1 1 130px; min-width: 110px; background: #FFFFFF; color: var(--texto); font-family: 'Montserrat', sans-serif; cursor: pointer; outline: none; }
...
.empleados-table th { background: var(--fondo); padding: 8px 12px; text-align: left; font-weight: 600; font-size: 0.8rem; color: var(--texto); border-bottom: 1px solid var(--borde); font-family: 'Montserrat', sans-serif; white-space: nowrap; }
.empleados-table td { padding: 7px 12px; border-bottom: 1px solid var(--borde); color: var(--texto); font-size: 0.86rem; font-family: 'Montserrat', sans-serif; }
...
/* ── Badges ── */
.badge-puesto { display: inline-block; padding: 2px 8px; border-radius: 20px; font-size: 0.76rem; font-weight: 600; background: #EDE9FE; color: #7C3AED; }
...
.badge-docente { display: inline-block; padding: 2px 8px; border-radius: 20px; font-size: 0.76rem; font-weight: 600; background: #DBEAFE; color: #1B396A; }
...
.modal-content { background: #FFFFFF; width: 520px; max-width: 92%; border-radius: 12px; box-shadow: 0 20px 50px rgba(0,0,0,0.18); overflow: hidden; border: 1px solid var(--borde); }
...
.btn-cerrar-modal { background: none; border: none; color: white; font-size: 1.6rem; cursor: pointer; line-height: 1; opacity: 0.85; transition: opacity 0.15s; }
