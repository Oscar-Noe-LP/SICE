...
<template>
  <MainLayout v-slot="{ busquedaGlobal }">
    <div class="solicitudes-page">
      ...
      <!-- Filtros (estilo SesionesComiteView) -->
      ...
      <!-- Modal Filtros Avanzados (igual que SesionesComiteView) -->
      ...
      <div class="tabla-card">
        ...
        <!-- Paginación (igual que SesionesComiteView) -->
...
// ── Carga de solicitudes (SOLO UNA VEZ, sin filtros en API) ─
const cargarSolicitudes = async () => {
  cargando.value = true
  errorCarga.value = false
  try {
    // Cargamos TODAS las solicitudes, el filtrado se hace en frontend
...
// ── Filtrado LOCAL (igual que SesionesComiteView) ───────
...
// ── Acciones de filtros (igual que SesionesComiteView) ──
const aplicarFiltros = () => {
  reiniciarPagina()
  mostrarFiltrosModal.value = false
  // No hay llamada al API, el computed se recalcula automáticamente
}
...
.titulo-pagina { 
  color: #1A1A1A; font-size: 1.9rem; font-weight: 800; margin: 0 0 0.25rem; 
}
...
/* ============ FILTROS (IDÉNTICO A SESIONESCOMITEVIEW) ============ */
.filtros-card { 
  background: #FFFFFF; 
  border-radius: 12px; 
...
/* ============ MODAL FILTROS (IDÉNTICO A SESIONESCOMITEVIEW) ============ */
.modal-fondo { 
  position: fixed; 
  inset: 0; 
...
/* ============ TABLA (IDÉNTICO A SESIONESCOMITEVIEW) ============ */
.tabla-card { 
  background: #FFFFFF; 
  border-radius: 12px; 
  border: 1px solid #E5E7EB; 
...
/* ============ PAGINACIÓN (IDÉNTICO A SESIONESCOMITEVIEW) ============ */
.paginacion-wrap { 
  padding: 0.8rem 1.4rem; 
  border-top: 1px solid #E5E7EB; 
  display: flex; 
