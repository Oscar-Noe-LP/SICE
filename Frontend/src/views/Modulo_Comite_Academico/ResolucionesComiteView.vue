...
<div><h1 class="titulo-pagina">Resoluciones</h1><p class="subtitulo">Registro de decisiones emitidas por el comité académico</p></div>
<button @click="abrirModalNueva" class="btn-primario"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="18" height="18"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Nueva Resolución</button>
</div>
...
<input v-model="busqueda" type="text" placeholder="Buscar por folio o nombre..." class="input-busqueda" @input="reiniciarPagina()" />
...
<div class="modal-cabecera"><h3>Filtros de Resoluciones</h3><button @click="mostrarFiltrosModal = false" class="btn-cerrar-modal"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="20" height="20"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>
<div class="modal-cuerpo filtros-grid">
<div class="campo-form">
<label class="campo-label">Sesión</label>
<select v-model="filtroSesion" class="campo-input">
...
<div class="tabla-encabezado"><span class="tabla-contador">{{ resolucionesFiltradas.length }} resolución(es) · Página {{ paginaActual }} de {{ totalPaginas }}</span></div>
<div class="tabla-scroll">
<table class="tabla-principal tabla-densa">
<thead><tr><th>Folio</th><th>Solicitante</th><th>Tipo</th><th>Sesión</th><th>Decisión</th><th>Fecha</th><th class="centrado">Acciones</th></tr></thead>
<tbody>
<tr v-for="res in resolucionesPaginadas" :key="res.id">
<td><span class="folio-chip">{{ res.folio_solicitud }}</span></td>
<td><div class="persona-info"><div class="persona-avatar">{{ iniciales(res.solicitante) }}</div><span class="texto-principal">{{ res.solicitante }}</span></div></td>
<td class="texto-secundario">{{ res.tipo }}</td>
<td class="texto-secundario sesion-texto">{{ formatearFechaCorta(res.fecha_sesion) }}</td>
<td><p class="decision-texto">{{ res.decision }}</p></td>
<td class="texto-secundario">{{ formatearFechaCorta(res.fecha) }}</td>
<td class="centrado">
<div class="acciones-fila">
<button @click="verDetalle(res)" class="btn-accion ver" title="Ver detalle"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
</div>
</td>
</tr>
<tr v-if="resolucionesPaginadas.length === 0"><td colspan="7" class="sin-resultados"><svg viewBox="0 0 24 24" fill="none" stroke="#D6D6D6" stroke-width="1.5" width="40" height="40"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><p>No se encontraron resoluciones</p></td></tr>
</tbody>
</table>
</div>
...
<div class="modal-caja">
<div class="modal-cabecera"><h3>Nueva Resolución</h3><button @click="cerrarModal" class="btn-cerrar-modal"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="20" height="20"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>
<div class="modal-cuerpo">
<div class="campo-form"><label class="campo-label">Sesión <span class="requerido">*</span></label><select v-model="formModal.sesion_id" class="campo-input" :class="{ 'campo-error': errModal.sesion_id }"><option value="">Selecciona una sesión</option><option v-for="s in sesiones" :key="s.id" :value="s.id">{{ formatearFechaCorta(s.fecha) }} — {{ s.descripcion }}</option></select><span v-if="errModal.sesion_id" class="mensaje-error">{{ errModal.sesion_id }}</span></div>
<div class="campo-form"><label class="campo-label">Solicitud Pendiente <span class="requerido">*</span></label><select v-model="formModal.solicitud_id" class="campo-input" :class="{ 'campo-error': errModal.solicitud_id }"><option value="">Selecciona una solicitud</option><option v-for="s in solicitudesSinRes" :key="s.id" :value="s.id">{{ s.folio }} — {{ s.solicitante }}</option></select><span v-if="errModal.solicitud_id" class="mensaje-error">{{ errModal.solicitud_id }}</span><span v-if="solicitudesSinRes.length === 0" class="campo-nota-aviso">No hay solicitudes pendientes</span></div>
<div class="campo-form"><label class="campo-label">Decisión <span class="requerido">*</span></label><textarea v-model="formModal.decision" rows="4" placeholder="Describe la decisión..." class="campo-input campo-textarea" :class="{ 'campo-error': errModal.decision }"></textarea><span v-if="errModal.decision" class="mensaje-error">{{ errModal.decision }}</span></div>
</div>
<div class="modal-pie"><button @click="cerrarModal" class="btn-cancelar">Cancelar</button><button @click="guardarResolucion" class="btn-guardar" :disabled="cargando"><span v-if="cargando" class="spinner"></span>{{ cargando ? 'Registrando...' : 'Registrar Resolución' }}</button></div>
</div>
</div>
</transition>
...
const busqueda = ref('')
const filtroSesion = ref(''); const filtroTipo = ref('')
const mostrarFiltrosModal = ref(false)
const mostrarModal = ref(false)
const tiposSolicitud = ref([]); const sesiones = ref([]); const solicitudesSinRes = ref([]); const resoluciones = ref([])
const formModal = ref({ sesion_id: '', solicitud_id: '', decision: '' }); const errModal = ref({ sesion_id: '', solicitud_id: '', decision: '' })
...
onMounted(() => {
  ...
  if(route.query.solicitud) { formModal.value.solicitud_id=route.query.solicitud; mostrarModal.value=true }
  cargarTipos(); cargarSesiones(); cargarSolicitudesSinResolucion(); cargarResoluciones()
})

const resolucionesFiltradas = computed(() => resoluciones.value.filter(r => {
...
const totalPaginas = computed(() => Math.max(1, Math.ceil(resolucionesFiltradas.value.length / registrosPorPagina)))
const resolucionesPaginadas = computed(() => { const i=(paginaActual.value-1)*registrosPorPagina; return resolucionesFiltradas.value.slice(i, i+registrosPorPagina) })
const reiniciarPagina = () => { paginaActual.value=1 }
const cambiarPagina = (p) => { if(p>=1 && p<=totalPaginas.value) paginaActual.value=p }

const hayFiltrosActivos = computed(() => filtroSesion.value || filtroTipo.value)
const filtrosActivosCount = computed(() => [filtroSesion.value, filtroTipo.value].filter(Boolean).length)

const aplicarFiltros = () => { reiniciarPagina(); filtrar(); mostrarFiltrosModal.value = false }
const limpiarFiltros = () => { filtroSesion.value=''; filtroTipo.value=''; reiniciarPagina(); filtrar() }
const filtrar = async () => {
  cargando.value=true; errorCarga.value=false
  try {
<<<<<<< HEAD
...
const formatearFechaCorta = (f) => { if(!f) return '—'; const [a, mes, d] = f.split('-'); const meses = ['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic']; return `${parseInt(d)} ${meses[parseInt(mes)-1]} ${a}` }
...
.icono-busqueda { color: #6B7280; flex-shrink: 0; } .input-busqueda { border: none; background: transparent; padding: 9px 0; font-size: 0.875rem; font-family: inherit; outline: none; flex: 1; color: #1A1A1A; }
.input-busqueda::placeholder { color: #9CA3AF; } .btn-limpiar { background: none; border: none; color: #6B7280; cursor: pointer; padding: 2px; display: flex; align-items: center; }
...
.folio-chip { background: #DBEAFE; color: #1B396A; padding: 3px 8px; border-radius: 6px; font-family: monospace; font-size: 0.8rem; font-weight: 700; }
.persona-info { display: flex; align-items: center; gap: 8px; } .persona-avatar { width: 28px; height: 28px; background: #DBEAFE; color: #1B396A; border-radius: 6px; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 0.7rem; flex-shrink: 0; }
.texto-principal { font-weight: 600; color: #1A1A1A; font-size: 0.82rem; } .texto-secundario { color: #6B7280; font-size: 0.8rem; } .sesion-texto { white-space: nowrap; }
.decision-texto { font-size: 0.78rem; color: #1A1A1A; margin: 0; display: -webkit-box; -webkit-line-clamp: 2; line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; max-width: 250px; }
...
.btn-accion:hover { transform: scale(1.1); } .btn-accion.ver { background: #DBEAFE; color: #1B396A; }
...
/* Modal Nueva Resolución */
.modal-caja.modal-ancho { width: 520px; }
