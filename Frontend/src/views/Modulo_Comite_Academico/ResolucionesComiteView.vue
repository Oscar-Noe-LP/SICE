            ...
            <div class="modal-cuerpo filtros-grid">
              ...
              <div class="campo-form">
                ...
                <select v-model="filtroTipo" class="campo-input">
                  ...
                  <option v-for="t in tiposSolicitud" :key="t.id" :value="t.nombre">
                    {{ t.nombre }}
                  </option>
                </select>
              </div>
      ...
      <div class="tabla-card">
        <div class="tabla-encabezado">
          <span class="tabla-contador">
            {{ resolucionesFiltradas.length }} resolución(es) · Página {{ paginaActual }} de {{ totalPaginas }}
          </span>
        </div>
        <div class="tabla-scroll">
          <table class="tabla-principal tabla-densa">
            ...
            <tbody>
              <tr v-for="res in resolucionesPaginadas" :key="res.id">
                ...
                <td class="centrado">
                  <div class="acciones-fila">
                    <button @click="verDetalle(res)" class="btn-accion ver" title="Ver detalle">
              ...
              <tr v-if="resolucionesPaginadas.length === 0">
          ...
          <div class="modal-caja">
            ...
            <div class="modal-cuerpo">
              <div class="campo-form">
                ...
                <select v-model="formModal.sesion_id" class="campo-input" :class="{ 'campo-error': errModal.sesion_id }">
              ...
              <div class="campo-form">
                <label class="campo-label">Solicitud Pendiente <span class="requerido">*</span></label>
                <select v-model="formModal.solicitud_id" class="campo-input" :class="{ 'campo-error': errModal.solicitud_id }">
                ...
                <span v-if="solicitudesSinRes.length === 0" class="campo-nota-aviso">No hay solicitudes pendientes</span>
              </div>
              <div class="campo-form">
                <label class="campo-label">Decisión <span class="requerido">*</span></label>
                <textarea 
...
const resolucionesFiltradas = computed(() => 
  resoluciones.value.filter(r => {
    ...
    const coincide = !q || 
      r.folio_solicitud?.toLowerCase().includes(q) || 
...
const totalPaginas = computed(() => 
  Math.max(1, Math.ceil(resolucionesFiltradas.value.length / registrosPorPagina))
)

const resolucionesPaginadas = computed(() => {
  const inicio = (paginaActual.value - 1) * registrosPorPagina
  return resolucionesFiltradas.value.slice(inicio, inicio + registrosPorPagina)
})

const hayFiltrosActivos = computed(() => filtroSesion.value || filtroTipo.value)
const filtrosActivosCount = computed(() => 
  [filtroSesion.value, filtroTipo.value].filter(Boolean).length
)
...
const guardarResolucion = async () => {
  ...
  try {
    ...
    mostrarNotificacion(error.message || 'Error al registrar la resolución', 'error')
  } finally {
    cargando.value = false
  }
}
...
const formatearFechaCorta = (f) => {
  ...
  return `${parseInt(d)} ${meses[parseInt(mes)-1]} ${a}`
}
...
const verDetalle = (res) => {
  router.push(`/comite/resoluciones/${res.id}`)
}
</script>
