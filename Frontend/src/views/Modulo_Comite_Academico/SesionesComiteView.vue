...
<div><h1 class="titulo-pagina">Sesiones del Comité</h1><p class="subtitulo">Registro y seguimiento de sesiones del comité académico</p></div>
<button @click="abrirModalNueva" class="btn-primario"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="18" height="18"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Nueva Sesión</button>
</div>

<!-- Filtros: Barra de búsqueda + botón Filter Icon -->
...
<input v-model="busqueda" type="text" placeholder="Buscar por descripción..." class="input-busqueda" @input="reiniciarPagina()" />
<button v-if="busqueda" @click="busqueda = ''; reiniciarPagina(); filtrar()" class="btn-limpiar"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="14" height="14"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
</div>
...
<div class="modal-cabecera"><h3>Filtros de Sesiones</h3><button @click="mostrarFiltrosModal = false" class="btn-cerrar-modal"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="20" height="20"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>
...
<div class="tabla-encabezado"><span class="tabla-contador">{{ sesionesFiltradas.length }} sesión(es) · Página {{ paginaActual }} de {{ totalPaginas }}</span></div>
<div class="tabla-scroll">
<table class="tabla-principal tabla-densa">
<thead><tr><th>Fecha</th><th>Descripción</th><th class="centrado">Acta/Resoluciones</th><th class="centrado">Acciones</th></tr></thead>
<tbody>
...
<td class="centrado"><span class="badge-resoluciones" :style="ses.resoluciones > 0 ? { background: '#DCFCE7', color: '#16A34A' } : { background: '#F3F4F6', color: '#6B7280' }">{{ ses.resoluciones }} resolución(es)</span></td>
<td class="centrado">
<div class="acciones-fila">
<button @click="router.push(`/comite/resoluciones?sesion=${ses.id}`)" class="btn-accion ver" title="Ver resoluciones"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
<button @click="abrirModalEditar(ses)" class="btn-accion editar" title="Editar sesión"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg></button>
<button @click="eliminarSesion(ses)" class="btn-accion eliminar" title="Eliminar sesión"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg></button>
</div>
</td>
</tr>
<tr v-if="sesionesPaginadas.length === 0"><td colspan="4" class="sin-resultados"><svg viewBox="0 0 24 24" fill="none" stroke="#D6D6D6" stroke-width="1.5" width="40" height="40"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/></svg><p>Sin sesiones registradas</p></td></tr>
</tbody>
</table>
</div>
...
<div class="modal-caja">
<div class="modal-cabecera"><h3>{{ modoEdicion ? 'Editar Sesión' : 'Nueva Sesión' }}</h3><button @click="cerrarModal" class="btn-cerrar-modal"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="20" height="20"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>
<div class="modal-cuerpo">
<div class="campo-form"><label class="campo-label">Fecha <span class="requerido">*</span></label><div class="campo-fecha-wrap"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16" class="icono-campo"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg><input v-model="formModal.fecha" type="date" class="campo-input campo-input-fecha" :class="{ 'campo-error': errModal.fecha }" /></div><span v-if="errModal.fecha" class="mensaje-error">{{ errModal.fecha }}</span></div>
<div class="campo-form"><label class="campo-label">Descripción <span class="requerido">*</span></label><textarea v-model="formModal.descripcion" rows="3" placeholder="Ej: Sesión ordinaria de abril 2026..." class="campo-input campo-textarea" :class="{ 'campo-error': errModal.descripcion }"></textarea><span v-if="errModal.descripcion" class="mensaje-error">{{ errModal.descripcion }}</span></div>
</div>
<div class="modal-pie"><button @click="cerrarModal" class="btn-cancelar">Cancelar</button><button @click="guardarSesion" class="btn-guardar" :disabled="cargando"><span v-if="cargando" class="spinner"></span>{{ cargando ? 'Guardando...' : (modoEdicion ? 'Actualizar' : 'Registrar Sesión') }}</button></div>
</div>
</div>
</transition>
...
const cargando = ref(false); const errorCarga = ref(false)
const busqueda = ref('')
const filtroFecha = ref(''); const filtroActa = ref('')
const mostrarFiltrosModal = ref(false)
const mostrarModal = ref(false); const modoEdicion = ref(false); const sesionEditando = ref(null)
const sesiones = ref([])
const formModal = ref({ fecha: '', descripcion: '' }); const errModal = ref({ fecha: '', descripcion: '' })
...
const sesionesFiltradas = computed(() => sesiones.value.filter(s => {
  ...
  const coincideActa = !filtroActa.value || (s.numero_acta && String(s.numero_acta).toLowerCase().includes(filtroActa.value.toLowerCase()))
  return coincideDesc && coincideFecha && coincideActa
}))
const paginaActual = ref(1); const registrosPorPagina = 10
const totalPaginas = computed(() => Math.max(1, Math.ceil(sesionesFiltradas.value.length / registrosPorPagina)))
const sesionesPaginadas = computed(() => { const i=(paginaActual.value-1)*registrosPorPagina; return sesionesFiltradas.value.slice(i, i+registrosPorPagina) })
...
const filtrosActivosCount = computed(() => [filtroFecha.value, filtroActa.value].filter(Boolean).length)

const aplicarFiltros = () => { reiniciarPagina(); mostrarFiltrosModal.value = false }
const limpiarFiltros = () => { filtroFecha.value=''; filtroActa.value=''; reiniciarPagina() }
const filtrar = () => { reiniciarPagina() }

const abrirModalNueva = () => { modoEdicion.value=false; sesionEditando.value=null; formModal.value={fecha:'',descripcion:''}; errModal.value={fecha:'',descripcion:''}; mostrarModal.value=true }
const abrirModalEditar = (ses) => { modoEdicion.value=true; sesionEditando.value=ses; formModal.value={fecha:ses.fecha, descripcion:ses.descripcion}; errModal.value={fecha:'',descripcion:''}; mostrarModal.value=true }
...
const eliminarSesion = async (ses) => {
  ...
  cargando.value=true
  try {
    const res=await fetch(`http://localhost:8000/api/comite/sesiones/${ses.id}`, { method:'DELETE' })
...
const formatearFechaLarga = (f) => { if(!f) return '—'; const [a, mes, d] = f.split('-'); const meses = ['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre']; return `${parseInt(d)} de ${meses[parseInt(mes)-1]} de ${a}` }
</script>
...
/* Filtros: barra búsqueda + botón icono */
...
.busqueda-wrap { display: flex; align-items: center; gap: 8px; background: #F5F5F5; border: 1px solid #E5E7EB; border-radius: 8px; padding: 0 12px; flex: 1; min-width: 180px; }
.busqueda-wrap:focus-within { border-color: #1B396A; background: #DBEAFE; } .icono-busqueda { color: #6B7280; flex-shrink: 0; }
.input-busqueda { border: none; background: transparent; padding: 9px 0; font-size: 0.875rem; font-family: inherit; outline: none; flex: 1; color: #1A1A1A; } .input-busqueda::placeholder { color: #9CA3AF; }
...
.filtros-activos-indicador { width: 22px; height: 22px; background: #1B396A; color: #FFFFFF; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 700; cursor: pointer; }
...
.tabla-principal td.centrado { text-align: center; } .tabla-principal tr:last-child td { border-bottom: none; } .tabla-principal tr:hover { background: #F9FAFB; }
.texto-principal { font-weight: 600; color: #1A1A1A; font-size: 0.82rem; } .texto-secundario { color: #6B7280; font-size: 0.8rem; }
.badge-resoluciones { font-size: 0.7rem; font-weight: 700; padding: 2px 8px; border-radius: 20px; }
...
.btn-accion:hover { transform: scale(1.1); } .btn-accion.ver { background: #DBEAFE; color: #1B396A; } .btn-accion.editar { background: #F3F4F6; color: #4B5563; } .btn-accion.eliminar { background: #FEE2E2; color: #DC2626; }
...
.btn-pag { padding: 6px 12px; border: 1px solid #E5E7EB; background: #FFFFFF; border-radius: 6px; font-size: 0.8rem; cursor: pointer; color: #4B5563; } .btn-pag:disabled { opacity: 0.5; cursor: not-allowed; } .btn-pag:hover:not(:disabled) { background: #F3F4F6; }
.pag-numeros { display: flex; gap: 4px; } .btn-pag-num { width: 28px; height: 28px; border-radius: 6px; border: 1px solid #E5E7EB; background: #FFFFFF; font-size: 0.8rem; cursor: pointer; display: flex; align-items: center; justify-content: center; } .btn-pag-num.activa { background: #1B396A; color: #FFFFFF; border-color: #1B396A; } .btn-pag-num:hover:not(.activa) { background: #F3F4F6; }

/* Modal Nueva/Editar Sesión */
.campo-fecha-wrap { position: relative; display: flex; align-items: center; } .icono-campo { position: absolute; left: 12px; color: #6B7280; pointer-events: none; }
.campo-input-fecha { padding-left: 38px; } .campo-textarea { resize: vertical; min-height: 80px; } .mensaje-error { font-size: 0.78rem; color: #DC2626; font-weight: 500; }
.campo-input.campo-error { border-color: #DC2626; }
.spinner { width: 14px; height: 14px; border-radius: 50%; border: 2px solid rgba(255,255,255,0.3); border-top-color: #FFFFFF; animation: spin 0.7s linear infinite; } @keyframes spin { to { transform: rotate(360deg); } }
...
@media (max-width: 640px) { .filtros-grid { grid-template-columns: 1fr; } .paginacion-wrap { flex-direction: column; gap: 0.5rem; } }
</style>