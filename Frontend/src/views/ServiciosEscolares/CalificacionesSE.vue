  ...
  <div class="calificaciones-page">
    ...
    <div class="breadcrumb">
      ...
      <template v-else-if="vista === 'tabla' && grupoSeleccionado">
        <span class="breadcrumb-link" @click="volverACarreras" style="cursor:pointer;">Calificaciones</span>
        <span class="sep">›</span>
        <span class="breadcrumb-link" @click="volverACarrera" style="cursor:pointer;">{{ carreraSeleccionada?.nombre }}</span>
        <span class="sep">›</span>
      ...
      <div class="filtros-card" style="margin-bottom: 1.5rem;">
        <div class="busqueda-wrapper" style="max-width: 400px;">
          <div class="busqueda-control">
            ...
            <input v-model="busquedaCarrera" type="text" placeholder="Buscar carrera..." class="input-busqueda-control" />
          </div>
        </div>
      </div>
    ...
    <div v-if="vista === 'carrera' && carreraSeleccionada">
      ...
      <div class="filtros-card" style="margin-bottom: 1.5rem;">
        <div class="busqueda-wrapper" style="max-width: 100%;">
          <div class="busqueda-control">
            ...
            <input v-model="busquedaCarreraDetalle" type="text" placeholder="Buscar materia o docente..." class="input-busqueda-control" />
          </div>
        </div>
      </div>
        ...
        <div class="tabla-card">
          <div class="tabla-scroll">
            <table class="tabla-califs tabla-grupos">
              <thead>
                <tr>
                  <th>Clave</th><th>Materia</th><th class="centrado">Semestre</th>
                  <th>Docente</th><th class="centrado">Alumnos</th><th class="centrado">Acción</th>
                </tr>
              </thead>
              <tbody>
                <tr v-if="cargandoGruposCarrera">
                  <td colspan="6" class="sin-resultados"><p>Cargando grupos...</p></td>
                </tr>
                <tr v-for="grupo in gruposCarreraFiltrados" :key="grupo.id_grupo">
                  <td><span class="control-chip">{{ grupo.clave_grupo }}</span></td>
                  <td>{{ grupo.materia }}</td>
                  <td class="centrado"><span class="semestre-badge">{{ grupo.semestre ?? '—' }}</span></td>
                ...
                <tr v-if="!cargandoGruposCarrera && gruposCarreraFiltrados.length === 0">
                  <td colspan="6" class="sin-resultados"><p>No se encontraron grupos</p></td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div class="actas-seccion" style="margin-top: 2rem;">
        ...
        <div class="mini-resumen-grid" style="margin-bottom: 1rem;">
          <div class="mini-resumen-item"><span class="mini-resumen-numero color-verde">{{ kpisActas.generadas }}</span><span class="mini-resumen-etiqueta">Generadas</span></div>
          <div class="mini-resumen-item"><span class="mini-resumen-numero" style="color:#F59E0B;">{{ kpisActas.pendientes }}</span><span class="mini-resumen-etiqueta">Pendientes</span></div>
          <div class="mini-resumen-item"><span class="mini-resumen-numero color-rojo">{{ kpisActas.sinCalificacion }}</span><span class="mini-resumen-etiqueta">Sin calificación</span></div>
          <div class="mini-resumen-item"><span class="mini-resumen-numero">{{ actas.length }}</span><span class="mini-resumen-etiqueta">Total</span></div>
        </div>
        <div class="tabla-card">
          <div class="tabla-scroll">
            <table class="tabla-califs">
              <thead>
                <tr><th>Grupo</th><th>Materia</th><th>Docente</th><th class="centrado">Estatus</th><th class="centrado">Acciones</th></tr>
              </thead>
              <tbody>
                <tr v-if="cargandoActas"><td colspan="5" class="sin-resultados"><p>Cargando actas...</p></td></tr>
                <tr v-else-if="actas.length === 0"><td colspan="5" class="sin-resultados"><p>No hay actas disponibles</p></td></tr>
        ...
        <div class="docentes-grid">
          <div v-for="docente in docentesCarreraFiltrados" :key="docente.nombre" class="docente-card">
            ...
            <div class="docente-card-footer">
              <button @click="abrirModalDocente(docente)" class="btn-accion ver grande" title="Ver detalles">
    ...
    <!-- ══ PANTALLA 3: TABLA DE CALIFICACIONES ══ -->
    <div v-if="vista === 'tabla' && grupoSeleccionado">
      ...
      <div class="filtros-card">
        <div class="filtros-container">
          <div class="busqueda-wrapper">
            <div class="busqueda-control">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16" class="icono-lupa"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              <input v-model="busquedaControl" type="text" placeholder="Buscar por nombre o control..." class="input-busqueda-control" @input="buscarEnTiempoReal" />
            </div>
          </div>
      ...
      <div class="kpis-grupo-grid">
        <div class="kpi-item"><span class="kpi-valor" :class="clasePromedio(promedioGeneral)">{{ promedioGeneral }}</span><span class="kpi-label">Promedio grupo</span></div>
        <div class="kpi-item"><span class="kpi-valor kpi-verde">{{ alumnos.length - totalReprobados - totalNC }}</span><span class="kpi-label">Aprobados</span></div>
        <div class="kpi-item"><span class="kpi-valor kpi-rojo">{{ totalReprobados }}</span><span class="kpi-label">Reprobados</span></div>
        <div class="kpi-item"><span class="kpi-valor kpi-gris">{{ totalNC }}</span><span class="kpi-label">Sin calificar</span></div>
      </div>

      <div class="subtab-nav">
        <button class="subtab-btn" :class="{ activo: vistaTabla === 'calificaciones' }" @click="vistaTabla = 'calificaciones'">
        ...
        <button class="subtab-btn" :class="{ activo: vistaTabla === 'especiales' }" @click="vistaTabla = 'especiales'">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2"/></svg>
          Especiales / Globales
        </button>
      </div>
      ...
      <div v-if="vistaTabla === 'calificaciones'" class="tabla-card">
        <div class="tabla-encabezado">
          <h3 class="seccion-titulo sin-margen">Registro de Calificaciones</h3>
        ...
        <div class="tabla-scroll">
          <table class="tabla-califs" @keydown="navegarTeclado">
            <thead>
              <tr>
                <th>No. de Control</th><th>Nombre del Alumno</th>
                ...
                <th class="centrado">Final</th><th class="centrado">Estado</th><th class="centrado">Acciones</th>
              </tr>
            </thead>
            <tbody>
              <tr
                ...
                :key="alumno.control"
                :class="[{ 'fila-activa': filaActiva === index && !esNC(alumno) }, { 'fila-reprobado': Number(calcularFinal(alumno)) < 60 && !esNC(alumno) }, { 'fila-sin-calificar': esNC(alumno) }]"
              ...
              >
                ...
                <td class="centrado"><div class="input-nota-wrap"><input v-model="alumno.p1" type="number" step="0.01" min="0" max="100" class="input-nota" :class="claseNota(alumno.p1)" @focus="filaActiva = index" /></div></td>
                <td class="centrado"><div class="input-nota-wrap"><input v-model="alumno.p2" type="number" step="0.01" min="0" max="100" class="input-nota" :class="claseNota(alumno.p2)" @focus="filaActiva = index" /></div></td>
                <td class="centrado"><div class="input-nota-wrap"><input v-model="alumno.proy" type="number" step="0.01" min="0" max="100" class="input-nota" :class="claseNota(alumno.proy)" @focus="filaActiva = index" /></div></td>
                <td class="centrado">
        ...
        <div class="paginacion-container" v-if="alumnosFiltrados.length > 0">
          ...
          <div class="paginacion-centro"><span class="paginacion-pagina-label">Página {{ paginaActual }} de {{ totalPaginas }}</span></div>
          <div class="paginacion-controles">
            ...
            <template v-for="(item, idx) in paginasVisibles" :key="idx">
              ...
              <button v-else @click="cambiarPagina(item)" :class="['btn-num', { activa: paginaActual === item }]">{{ item }}</button>
            </template>
        ...
        <div class="tabla-resumen">
          <div class="resumen-item"><span class="ri-label">Promedio grupo:</span><strong :class="clasePromedio(promedioGeneral)">{{ promedioGeneral }}</strong></div>
          <div class="resumen-separador"></div>
          <div class="resumen-item"><span class="ri-label">Reprobados:</span><strong class="color-rojo">{{ totalReprobados }} / {{ alumnos.length }}</strong></div>
          <div class="resumen-separador"></div>
          <div class="resumen-item"><span class="ri-label">Sin calificar:</span><strong>{{ totalNC }}</strong></div>
          <div class="resumen-separador"></div>
          <div class="resumen-item"><span class="ri-label">Aprobados:</span><strong class="color-verde">{{ alumnos.length - totalReprobados - totalNC }}</strong></div>
        </div>
      </div>
      ...
      <div v-if="vistaTabla === 'especiales'" class="tabla-card">
        ...
        <div class="tabla-scroll">
          <table class="tabla-califs">
            <thead>
              <tr><th>Alumno</th><th class="centrado">Tipo</th><th class="centrado">Calificación</th><th class="centrado">Resultado</th><th class="centrado">Acción</th></tr>
            </thead>
            <tbody>
              <tr v-for="e in especiales" :key="e.id">
                <td>{{ e.alumno }}</td>
                <td class="centrado"><span class="tipo-badge" :class="e.tipo === 'especial' ? 'tipo-especial' : 'tipo-global'">{{ e.tipo === 'especial' ? 'Especial' : 'Global' }}</span></td>
                <td class="centrado"><div class="final-chip" :class="e.calificacion ? clasePromedio(e.calificacion) : 'promedio-sin-calificar'">{{ e.calificacion ?? '–' }}</div></td>
                <td class="centrado"><span class="badge-estado" :class="e.calificacion >= 60 ? 'excelente' : 'reprobado'">{{ e.calificacion >= 60 ? 'Aprobado' : 'Reprobado' }}</span></td>
                <td class="centrado acciones-cell">
      ...
      <div v-if="showModalAlumno" class="modal-overlay" @click.self="cerrarModalAlumno">
        <div class="modal-content modal-alumno-rediseno">
          <div class="modal-alumno-header">
            <div class="modal-alumno-avatar">{{ iniciales(alumnoSeleccionado?.nombre) }}</div>
          ...
          <div class="modal-tabs">
            <button :class="['modal-tab', { activa: modalTab === 'general' }]" @click="modalTab = 'general'">General</button>
            <button :class="['modal-tab', { activa: modalTab === 'calificaciones' }]" @click="modalTab = 'calificaciones'">Calificaciones</button>
            <button :class="['modal-tab', { activa: modalTab === 'historial' }]" @click="modalTab = 'historial'">Historial</button>
          </div>
          <div class="modal-body">
            <div v-if="modalTab === 'general' && alumnoSeleccionado" class="tab-general">
              <div class="info-grid">
                <div class="info-card"><div class="info-icono azul"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><div class="info-texto"><span class="info-label">Nombre</span><span class="info-valor">{{ alumnoSeleccionado.nombre }}</span></div></div>
                <div class="info-card"><div class="info-icono verde"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg></div><div class="info-texto"><span class="info-label">No. Control</span><span class="info-valor">{{ alumnoSeleccionado.control }}</span></div></div>
                <div class="info-card"><div class="info-icono morado"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c0 1.1 2.7 2 6 2s6-.9 6-2v-5"/></svg></div><div class="info-texto"><span class="info-label">Carrera</span><span class="info-valor">{{ carreraSeleccionada?.nombre || 'N/A' }}</span></div></div>
                <div class="info-card"><div class="info-icono naranja"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div><div class="info-texto"><span class="info-label">Semestre</span><span class="info-valor">{{ alumnoSeleccionado.semestre || 'N/A' }}</span></div></div>
              </div>
            ...
            <div v-if="modalTab === 'calificaciones' && alumnoSeleccionado" class="tab-calificaciones">
              <div class="calificaciones-grid">
                <div class="cal-card"><div class="cal-header"><span class="cal-titulo">Parcial 1</span><span class="cal-peso">30%</span></div><input v-model="alumnoSeleccionado.p1" type="number" step="0.01" min="0" max="100" class="input-nota grande" :class="claseNota(alumnoSeleccionado.p1)" /><div class="cal-barra"><div class="cal-barra-fill" :style="{ width: (Number(alumnoSeleccionado.p1 || 0)) + '%', background: colorNota(Number(alumnoSeleccionado.p1 || 0)) }"></div></div></div>
                <div class="cal-card"><div class="cal-header"><span class="cal-titulo">Parcial 2</span><span class="cal-peso">30%</span></div><input v-model="alumnoSeleccionado.p2" type="number" step="0.01" min="0" max="100" class="input-nota grande" :class="claseNota(alumnoSeleccionado.p2)" /><div class="cal-barra"><div class="cal-barra-fill" :style="{ width: (Number(alumnoSeleccionado.p2 || 0)) + '%', background: colorNota(Number(alumnoSeleccionado.p2 || 0)) }"></div></div></div>
                <div class="cal-card"><div class="cal-header"><span class="cal-titulo">Proyecto</span><span class="cal-peso">40%</span></div><input v-model="alumnoSeleccionado.proy" type="number" step="0.01" min="0" max="100" class="input-nota grande" :class="claseNota(alumnoSeleccionado.proy)" /><div class="cal-barra"><div class="cal-barra-fill" :style="{ width: (Number(alumnoSeleccionado.proy || 0)) + '%', background: colorNota(Number(alumnoSeleccionado.proy || 0)) }"></div></div></div>
              </div>
              <div class="final-display">
                <div class="final-label">Calificación Final</div>
                <div class="final-valor" :class="calcularFinal(alumnoSeleccionado) === null ? 'sin-calificar' : clasePromedio(calcularFinal(alumnoSeleccionado))">{{ calcularFinal(alumnoSeleccionado) ?? '–' }}</div>
              </div>
            ...
            <div v-if="modalTab === 'historial'" class="tab-historial">
              <div v-if="cargandoHistorial" class="sin-resultados-inner" style="padding:2rem;"><p style="color:#6B7280;font-size:0.9rem;">Cargando historial...</p></div>
              <div v-else-if="historialAlumno.length === 0" class="sin-resultados-inner" style="padding:2rem;"><p>No hay historial registrado.</p></div>
              <div v-else class="tabla-scroll">
                <table class="tabla-califs">
                  <thead><tr><th>Periodo</th><th>Materia</th><th class="centrado">Calificación</th><th class="centrado">Estatus</th></tr></thead>
                  <tbody>
                    <tr v-for="h in historialAlumno" :key="h.id ?? (h.periodo + h.materia)">
                      <td>{{ h.periodo }}</td><td>{{ h.materia }}</td>
                      <td class="centrado"><div class="final-chip" :class="h.calificacion ? clasePromedio(h.calificacion) : 'promedio-sin-calificar'">{{ h.calificacion ?? '–' }}</div></td>
                      <td class="centrado"><span class="badge-estado" :class="!h.calificacion ? 'sin-calificar' : Number(h.calificacion) >= 60 ? 'bien' : 'reprobado'">{{ !h.calificacion ? 'S/C' : Number(h.calificacion) >= 60 ? 'Aprobado' : 'Reprobado' }}</span></td>
                    </tr>
                  </tbody>
                </table>
              </div>
        ...
        <div class="modal-content modal-ver-docente">
          ...
          <div class="modal-body">
            <div class="docente-modal-header">
              <div class="docente-modal-avatar">{{ iniciales(docenteSeleccionado.nombre) }}</div>
              <div class="docente-modal-info"><h4>{{ docenteSeleccionado.nombre }}</h4><span class="docente-modal-carrera">{{ carreraSeleccionada?.nombre }}</span></div>
            </div>
            <div class="docente-modal-detalles">
              <div class="campo"><span class="campo-label">Materias impartidas</span><span class="campo-valor">{{ docenteSeleccionado.materias.join(', ') }}</span></div>
              <div class="campo"><span class="campo-label">Grupos asignados</span><span class="campo-valor">{{ docenteSeleccionado.grupos.join(', ') }}</span></div>
              <div class="campo"><span class="campo-label">Total de grupos</span><span class="campo-valor">{{ docenteSeleccionado.grupos.length }}</span></div>
            </div>
            <div class="docente-grupos" v-if="docenteSeleccionado.grupos.length > 0">
              <h4>Grupos que imparte</h4>
              <div class="tabla-scroll">
                <table class="tabla-califs">
                  <thead><tr><th>Clave</th><th>Materia</th><th class="centrado">Alumnos</th></tr></thead>
                  <tbody>
...
async function cargarGruposCarrera(idCarrera) {
  ...
  try {
    ...
    mostrarToast(err.response?.data?.message ?? 'Error al cargar grupos', 'error')
  } finally {
...
// ── Catálogos ──────────────────────────────────────────────────────────
const { carreras, cargandoCatalogos, errorCatalogos, cargarCatalogos } = useCatalogos()
...
const docentesCarrera = computed(() => {
  ...
  gruposCarrera.value.forEach(g => {
    ...
    if (!map.has(nombre)) map.set(nombre, { materias: [], grupos: [] })
    map.get(nombre).materias.push(g.materia)
    map.get(nombre).grupos.push(g.clave_grupo)
  })
  return Array.from(map, ([nombre, datos]) => ({ nombre, ...datos }))
})
...
async function cargarActas(idCarrera) {
  ...
  try { const { data } = await api.get(`/carreras/${idCarrera}/actas`); actas.value = data.actas ?? [] }
  catch { actas.value = []; mostrarToast('Error al cargar actas', 'error') }
  finally { cargandoActas.value = false }
}
async function generarActa(acta) {
  try { await api.post(`/actas/${acta.id_acta}/generar`); mostrarToast('Acta generada'); await cargarActas(carreraSeleccionada.value.id_carrera) }
  catch { mostrarToast('Error al generar acta', 'error') }
}
const descargarActa = (acta) => window.open(`/api/actas/${acta.id_acta}/pdf`, '_blank')
const imprimirActa  = (acta) => window.open(`/api/actas/${acta.id_acta}/pdf?print=1`, '_blank')

// ── Modal docente ──────────────────────────────────────────────────────
const showModalDocente = ref(false)
const docenteSeleccionado = ref(null)
const abrirModalDocente = (d) => { docenteSeleccionado.value = d; showModalDocente.value = true }
const cerrarModalDocente = () => { showModalDocente.value = false; docenteSeleccionado.value = null }
const getGruposPorDocente = (d) => gruposCarrera.value.filter(g => d.grupos.includes(g.clave_grupo))
...
const vistaTabla = ref('calificaciones')

async function cargarEspeciales(grupoId) {
  try { const { data } = await api.get(`/grupos/${grupoId}/especiales`); especiales.value = data.solicitudes ?? [] }
  catch { mostrarToast('Error al cargar especiales', 'error') }
}
...
async function cargarHistorial(control) {
  cargandoHistorial.value = true; historialAlumno.value = []
  try { const { data } = await api.get(`/alumnos/${control}/historial`); historialAlumno.value = data.historial ?? [] }
  catch { historialAlumno.value = [] }
...
const abrirModalAlumno = (a) => { alumnoSeleccionado.value = { ...a }; modalTab.value = 'general'; showModalAlumno.value = true; cargarHistorial(a.control) }
const cerrarModalAlumno = () => { showModalAlumno.value = false; alumnoSeleccionado.value = null }
...
const paginasVisibles = computed(() => {
  const total = totalPaginas.value; const actual = paginaActual.value
  if (total <= 7) return Array.from({ length: total }, (_, i) => i + 1)
  const paginas = []; const agregar = (p) => { if (!paginas.includes(p) && p >= 1 && p <= total) paginas.push(p) }
  agregar(1); agregar(actual - 1); agregar(actual); agregar(actual + 1); agregar(total)
  paginas.sort((a, b) => a - b)
  const resultado = []
  for (let i = 0; i < paginas.length; i++) { if (i > 0 && paginas[i] - paginas[i - 1] > 1) resultado.push('...'); resultado.push(paginas[i]) }
  return resultado
})
...
const claseNota = (val) => { const n = Number(val); if (!val && val !== 0) return ''; if (n >= 90) return 'nota-excelente'; if (n >= 70) return 'nota-bien'; if (n >= 60) return 'nota-regular'; return 'nota-baja' }
const clasePromedio = (val) => { const n = Number(val); if (n >= 90) return 'promedio-excelente'; if (n >= 70) return 'promedio-bien'; if (n >= 60) return 'promedio-regular'; return 'promedio-bajo' }
const colorNota = (n) => { if (n >= 90) return '#16A34A'; if (n >= 70) return '#1B396A'; if (n >= 60) return '#F59E0B'; return '#DC2626' }

// ── Navegación ─────────────────────────────────────────────────────────
const seleccionarCarrera = async (carrera) => {
  carreraSeleccionada.value = carrera; vista.value = 'carrera'
  await Promise.all([cargarGruposCarrera(carrera.id_carrera), cargarActas(carrera.id_carrera)])
}
const volverACarreras = () => { carreraSeleccionada.value = null; grupoSeleccionado.value = null; gruposDeCarrera.value = []; vista.value = 'carreras' }
const seleccionarGrupo = async (grupo) => {
  grupoSeleccionado.value = grupo; vista.value = 'tabla'; vistaTabla.value = 'calificaciones'; paginaActual.value = 1; filaActiva.value = null
  await Promise.all([cargarCalificacionesGrupo(grupo.id_grupo), cargarEspeciales(grupo.id_grupo)])
}
const volverACarrera = () => { grupoSeleccionado.value = null; vista.value = 'carrera' }

async function cargarCalificacionesGrupo(grupoId) {
  cargando.value = true
  try { const data = await getCalificacionesGrupo({ grupo: grupoId }); alumnos.value = data.alumnos ?? data }
  catch { alumnos.value = []; mostrarToast('Error al cargar calificaciones', 'error') }
...
const guardarDesdeModal = async () => {
  ...
  try {
    ...
    if (idx !== -1) { alumnos.value[idx].p1 = alumnoSeleccionado.value.p1; alumnos.value[idx].p2 = alumnoSeleccionado.value.p2; alumnos.value[idx].proy = alumnoSeleccionado.value.proy }
    await guardarCalificaciones([alumnoSeleccionado.value]); mostrarToast('Calificaciones guardadas')
  } catch { mostrarToast('Error al guardar', 'error') }
  finally { cargando.value = false }
}
...
const exportar = () => {
  ...
  const filas = datos.map(a => { const final = calcularFinal(a); return { 'No. Control': a.control, 'Nombre': a.nombre, 'Parcial 1 (30%)': a.p1 ?? '', 'Parcial 2 (30%)': a.p2 ?? '', 'Proyecto (40%)': a.proy ?? '', 'Final': final ?? '', 'Estado': final === null ? 'Sin calificar' : Number(final) >= 90 ? 'Excelente' : Number(final) >= 80 ? 'Bien' : Number(final) >= 60 ? 'Regular' : 'Reprobado' } })
  const ws = XLSX.utils.aoa_to_sheet(info)
  XLSX.utils.sheet_add_json(ws, filas, { origin: 'A4' })
  ws['!cols'] = [{ wch: 14 }, { wch: 30 }, { wch: 14 }, { wch: 14 }, { wch: 14 }, { wch: 8 }, { wch: 14 }]
  const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, clave.slice(0, 31))
  XLSX.writeFile(wb, `calificaciones_${clave}_${new Date().toISOString().slice(0,10)}.xlsx`)
  mostrarToast('Excel exportado correctamente')
}

// ── Paginación ─────────────────────────────────────────────────────────
const cambiarPagina = (p) => { if (p < 1 || p > totalPaginas.value) return; paginaActual.value = p; filaActiva.value = null; filasRef.value = [] }
const cambiarItemsPorPagina = (v) => { itemsPorPagina.value = Number(v); paginaActual.value = 1; filaActiva.value = null; filasRef.value = [] }
let debounceTimer = null
const buscarEnTiempoReal = () => { clearTimeout(debounceTimer); debounceTimer = setTimeout(() => { paginaActual.value = 1 }, 300) }
const navegarTeclado = (e) => {
  if (vista.value !== 'tabla') return
  const max = alumnosPaginados.value.length - 1
  if (e.key === 'ArrowDown') { e.preventDefault(); filaActiva.value = Math.min((filaActiva.value ?? -1) + 1, max); nextTick(() => filasRef.value[filaActiva.value]?.focus()) }
  else if (e.key === 'ArrowUp') { e.preventDefault(); filaActiva.value = Math.max((filaActiva.value ?? 1) - 1, 0); nextTick(() => filasRef.value[filaActiva.value]?.focus()) }
}
const atajoGlobal = (e) => { if (e.ctrlKey && e.key === 's' && vista.value === 'tabla') { e.preventDefault(); guardarTodo() } }

onMounted(async () => {
  try { await cargarCatalogos(['carreras']) } catch (err) { console.error(err) }
  window.addEventListener('keydown', atajoGlobal)
})
onUnmounted(() => window.removeEventListener('keydown', atajoGlobal))
const reintentarCatalogos = () => cargarCatalogos(['carreras'])
...
.calificaciones-page { width: 100%; max-width: 100%; font-family: 'Montserrat', sans-serif; box-sizing: border-box; }
.semestre-badge { display: inline-block; background: #EDE9FE; color: #7C3AED; font-weight: 700; font-size: 0.78rem; padding: 2px 10px; border-radius: 12px; min-width: 24px; text-align: center; }
...
.btn-limpiar-filtros { display: flex; align-items: center; gap: 6px; background: #FEF2F2; color: #DC2626; border: none; padding: 8px 14px; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; }
...
.input-nota.grande { width: 80px; padding: 8px; font-size: 1rem; margin: 0.5rem auto; display: block; }
...
.btn-accion:hover:not(:disabled) { transform: scale(1.1); } .btn-accion:disabled { opacity: 0.4; cursor: not-allowed; }
.btn-accion.guardar { background: #DCFCE7; color: #16A34A; } .btn-accion.ver { background: #DBEAFE; color: #1B396A; }
...
.btn-pag:hover:not(:disabled) { background: #F0F4FF; border-color: #BFDBFE; color: #1B396A; } .btn-pag:disabled { opacity: 0.35; cursor: not-allowed; }
.paginacion-ellipsis { width: 28px; height: 32px; display: flex; align-items: center; justify-content: center; color: #9CA3AF; font-size: 0.9rem; }
...
.btn-num:hover { background: #F0F4FF; border-color: #BFDBFE; color: #1B396A; } .btn-num.activa { background: #1B396A; color: #FFFFFF; border-color: #1B396A; box-shadow: 0 2px 6px rgba(27,57,106,0.3); }
...
.kpis-grupo-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 1.5rem; }
.kpi-item { background: #FFFFFF; border: 1px solid #E5E7EB; border-radius: 12px; padding: 1rem; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }
.kpi-valor { display: block; font-size: 1.8rem; font-weight: 800; color: #1A1A1A; }
.kpi-label { font-size: 0.78rem; color: #6B7280; font-weight: 600; margin-top: 4px; display: block; }
.kpi-verde { color: #16A34A; } .kpi-rojo { color: #DC2626; } .kpi-gris { color: #6B7280; }
.subtab-nav { display: flex; gap: 0.5rem; margin-bottom: 1rem; }
.subtab-btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 14px; border-radius: 8px; border: 1px solid #E5E7EB; background: #FFFFFF; color: #6B7280; font-size: 0.82rem; font-weight: 600; cursor: pointer; transition: all 0.2s; }
.subtab-btn:hover { background: #F0F4FF; color: #1B396A; border-color: #BFDBFE; }
.subtab-btn.activo { background: #1B396A; color: white; border-color: #1B396A; }
.tipo-badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 0.72rem; font-weight: 700; }
.tipo-especial { background: #EDE9FE; color: #7C3AED; } .tipo-global { background: #FEF3C7; color: #B45309; }
...
.modal-alumno-header { display: flex; align-items: center; gap: 1rem; padding: 1.5rem 1.5rem 1rem; background: linear-gradient(135deg, #1B396A 0%, #1D4ED8 100%); color: white; border-radius: 20px 20px 0 0; }
.modal-alumno-avatar { width: 56px; height: 56px; border-radius: 50%; background: rgba(255,255,255,0.2); display: flex; align-items: center; justify-content: center; font-size: 1.4rem; font-weight: 800; flex-shrink: 0; border: 2px solid rgba(255,255,255,0.4); }
.modal-alumno-titulo h2 { margin: 0; font-size: 1.3rem; font-weight: 700; }
.modal-alumno-control { font-size: 0.85rem; opacity: 0.9; font-family: monospace; }
.modal-alumno-titulo { flex: 1; }
.modal-tabs { display: flex; background: #F9FAFB; border-bottom: 1px solid #E5E7EB; }
.modal-tab { flex: 1; background: none; border: none; padding: 0.75rem 1rem; font-size: 0.85rem; font-weight: 600; color: #6B7280; cursor: pointer; border-bottom: 2px solid transparent; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 6px; font-family: inherit; }
.modal-tab.activa { color: #1B396A; border-bottom-color: #1B396A; background: #FFFFFF; }
.modal-close-btn { background: rgba(255,255,255,0.2); border: none; color: white; cursor: pointer; padding: 5px; border-radius: 8px; transition: background 0.2s; }
...
.info-icono.azul { background: #DBEAFE; color: #1B396A; } .info-icono.verde { background: #DCFCE7; color: #16A34A; } .info-icono.morado { background: #EDE9FE; color: #7C3AED; } .info-icono.naranja { background: #FFF7ED; color: #F59E0B; }
.info-texto { display: flex; flex-direction: column; }
.info-label { font-size: 0.75rem; color: #6B7280; font-weight: 600; }
.info-valor { font-weight: 700; color: #1A1A1A; font-size: 0.95rem; }
.estatus-card { background: #FFFFFF; border: 1px solid #E5E7EB; border-radius: 12px; padding: 1rem; display: flex; align-items: center; gap: 0.8rem; }
.estatus-card.aprobado { border-left: 4px solid #16A34A; } .estatus-card.reprobado { border-left: 4px solid #DC2626; } .estatus-card.sin-calificar { border-left: 4px solid #9CA3AF; }
.estatus-icono { width: 40px; height: 40px; border-radius: 10px; display: flex; align-items: center; justify-content: center; }
.estatus-card.aprobado .estatus-icono { background: #DCFCE7; color: #16A34A; } .estatus-card.reprobado .estatus-icono { background: #FEF2F2; color: #DC2626; } .estatus-card.sin-calificar .estatus-icono { background: #F3F4F6; color: #9CA3AF; }
.estatus-texto { display: flex; flex-direction: column; }
.estatus-texto span { font-size: 0.75rem; color: #6B7280; font-weight: 600; }
.estatus-texto strong { font-size: 1rem; color: #1A1A1A; }
.calificaciones-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem; }
.cal-card { background: #FFFFFF; border: 1px solid #E5E7EB; border-radius: 12px; padding: 1rem; text-align: center; }
.cal-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; }
.cal-titulo { font-weight: 600; font-size: 0.85rem; color: #1A1A1A; } .cal-peso { font-size: 0.75rem; color: #6B7280; font-weight: 600; }
.cal-barra { height: 8px; background: #E5E7EB; border-radius: 4px; overflow: hidden; margin-top: 0.5rem; }
.cal-barra-fill { height: 100%; border-radius: 4px; transition: width 0.4s ease; }
.final-display { background: #F9FAFB; border: 1px solid #E5E7EB; border-radius: 12px; padding: 1rem; text-align: center; margin-bottom: 1rem; }
.final-label { font-size: 0.8rem; color: #6B7280; font-weight: 600; margin-bottom: 0.3rem; }
.final-valor { font-size: 2rem; font-weight: 800; color: #1A1A1A; }
.final-valor.sin-calificar { color: #9CA3AF; }
.modal-acciones { display: flex; justify-content: flex-end; }
...
@media (max-width: 900px) { .mini-resumen-grid { grid-template-columns: repeat(2, 1fr); } .docentes-grid { grid-template-columns: repeat(2, 1fr); } .calificaciones-grid { grid-template-columns: 1fr; } .info-grid { grid-template-columns: 1fr; } }
@media (max-width: 640px) { .carreras-grid { grid-template-columns: 1fr; } .mini-resumen-grid { grid-template-columns: 1fr; } .filtros-container { flex-direction: column; align-items: stretch; } .tabla-encabezado { flex-direction: column; align-items: stretch; } .paginacion-container { flex-direction: column; align-items: center; } .modal-content { width: 95%; } .docentes-grid { grid-template-columns: 1fr; } .kpis-grupo-grid { grid-template-columns: repeat(2, 1fr); } }
</style>